...
You can visit http://52.91.26.66/ or http://ec2-52-91-26-66.compute-1.amazonaws.com/ for the website deployed.
...
### Step 1: Start a new Ubuntu Linux server instance on Amazon EC2 
  ...
  - Check for instance IPv4 public IP - 52.91.26.66
  - we can download a .pem file and connect with following command
    ```
     ssh -i 20_02_2019_957_Item_catalog.pem -p 2200 grader@52.91.26.66
...
- To check port 2200 wether working or not by ` ssh -i 20_02_2019_957_Item_catalog.pem -p 2200 grader@52.91.26.66
` 
...
### Step 7: Create an SSH key pair for `grader`
  ...
  - create .ssh folder by `sudo mkdir /home/grader/.ssh`
  ...
  - change permissions for .ssh folder `sudo chmod 0700 /home/grader/.ssh/`, for authorized_keys `sudo chmod 644 authorized_keys`
  - Check in `vi /etc/ssh/sshd_config` file if `PermitRootLogin` is set to `no`
    To exit from this file enter `:w !sudo tee %` and type `:!q`
  - Restart SSH: `sudo service ssh restart`
  - On the local machine, cheking if the grader account working or not by running this command : ` ssh -i 20_02_2019_957_Item_catalog.pem -p 2200 grader@52.91.26.66. `
...
- We need to change sqlite to postgresql create_engine in `__init__.py`,`database_setup.py` and `hoteldata.py`,
   ```
...
- Click `Credentials`.
- Create an OAuth Client ID (under the Credentials tab), and add http://52.91.26.66.xip.io and 
http://ec2-52-91-26-66.compute-1.amazonaws.com as authorized JavaScript 
origins.
- Add http://52.91.26.66.xip.io/login,http://52.91.26.66.xip.io/gconnect,http://52.91.26.66.xip.io/callback
...
### Step 13.2: Set up and enable a virtual host
  ...
  - Before running this code check your python version in `/var/www/catalog/catalog/venv3/lib`
  - Run this: `sudo vi /etc/apache2/sites-available/catalog.conf`
  - Paste this code: 
  ```
  <VirtualHost *:80>
      ServerName 52.91.26.66.xip.io
      ServerAlias ec2-52-91-26-66.compute-1.amazonaws.com
      ServerAdmin ubuntu@52.91.26.66
...
- Open your browser to http://35.175.152.204 or http://ec2-35-175-152-204.compute-1.amazonaws.com.
